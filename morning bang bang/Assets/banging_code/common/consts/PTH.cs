namespace banging_code.common
{
    public struct PTH
    {
        public const string ObstacleTilemapRendererPreset = "Presets/ObstacleTilemapRenderer";
        public const string FloorTilemapRendererPreset      = "Presets/FloorTilemapRenderer";
        public const string CCameraInstance                     = "Prefabs/CCamera";
        public const string PlayerAnimatorBundle               = "Configs/PlayerAnimatorBundle";
        public const string PauseMenuPrefab                     = "Prefabs/PauseMenu";
    }
}